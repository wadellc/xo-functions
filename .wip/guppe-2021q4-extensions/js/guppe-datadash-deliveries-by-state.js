/* Choropleth II - Deliveries by State
 * A retooling of Requests by State */

 jQuery(document).ready(function($) {
	const tooltipDBS = d3.select('div#dbs').append('div').attr('class', 'tooltipDBS').style('opacity', 0); //#dbs
	...
	const getMapPosition = () => {
		const elWidth = $('#delivery-map .map').width();
		return {
			mapWidth: elWidth,
	...
	const returnHTML = (data) => `
		...
		<div class="amount">
			${numberWithCommas(data.units_delivered)}
		</div>
		<div class="top-item">
			<div class="desc">TOP ITEM DELIVERED:</div>
			<div class="item">${data.top_item}</div>
		</div>`;

	// Create Event Handlers for mouse
	const handleMouseOver = (d) => {
		tooltipDBS.transition().duration(200).style('opacity', 1);
 		tooltipDBS.html(returnHTML(d))
	...
	const handleMouseOut = () => tooltipDBS.transition().duration(500).style('opacity', 0);

	// Set color for each state
	const handleSetColor = (d) => {
		if (d) {
			const amount = parseInt(d.units_delivered, 10);
			if (amount >= 0 && amount < 25000) return '#B2D2DD';
			if (amount >= 25000 && amount < 50000) return '#68B0C4'; // New DBS Value
			if (amount >= 50000 && amount < 100000) return '#4C8BBC'; // New DBS Value
			if (amount >= 100000 && amount < 250000) return '#2F6CB5'; // New DBS Value
			if (amount >= 250000 && amount < 500000) return '#22508A'; // New DBS Value
			if (amount > 500000) return '#0C2E4D'; // New DBS Value
		}
...
// 		const dataUrl = 'https://storage.googleapis.com/data-dashboard-backend/8_distribution_by_state'; // New DBS Value
...
// 		const staticAssetUri = 'https://cdn.getusppe.org/data-dashboard/assets/img/';
		...
		const dataUrl = 'https://getusppe.org/8_distribution_by_state';
		const polygonUrl = 'https://getusppe.org/2_polygon/';
		// this also contains what is effectively a dictionary of the icons that go with each item
		...
		const topRequested = items['allTime'].find((el) => el.item === data.top_item); 
		$('#delivery-map div.amount').empty();
		$('#delivery-map p.top-item-name').empty();
		$('#delivery-map div.amount').append(numberWithCommas(data.total_donations_delivered));
		$('#delivery-map p.top-item-name').append(data.top_item);
		$('#delivery-map div.top-item-icon img').attr('src', getImgUrl(topRequested.slug));

		const {mapWidth, mapHeight, mapScale, mapTranslateX, mapTranslateY, mapFontSize} = getMapPosition();
		const svg = d3.select('#delivery-map svg')
