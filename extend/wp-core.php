...
function custom_dynamic_admin_js_tweaks() {
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            ...
            // Change the Form Submit Button Text & Form Title Heading
            if ( parentId > 0 ) {
...
add_action( 'admin_footer', function() {
    // Wrap the hook execution inside an anonymous check to guarantee get_current_screen matches are initialized
...
add_action( 'manage_plugins_custom_column', function( $column_name, $plugin_file, $plugin_data ) {
    if ( $column_name === 'install_date' ) {
        ...
        if ( isset( $install_dates[$plugin_file] ) ) {
            echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $install_dates[$plugin_file] ) ) );
        } else {
